...
def train(train_sets, dev_sets, test_sets, unlabeled_sets, fold):
    ...
    D = DomainClassifier(opt.D_layers, opt.shared_hidden_size + opt.num_labels, opt.shared_hidden_size,
        ...
        for i in tqdm(range(num_iter)):
            ...
            for _ in range(n_critic):                
                ...
                for domain in opt.all_domains:
                    ...
                    domain_feat = F_d[domain](d_inputs)
                    features = torch.cat((shared_feat, domain_feat), dim=1)
                
                    outputs = C(features)
                    
                    
                    shared_feat = torch.cat((shared_feat, outputs), dim = 1)
